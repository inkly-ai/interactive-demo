...
import { CONFIG_PATH } from "@/components/demo-editor/codec";
...
export function EditorShell({ slug }: { slug: string }) {
    ...
    const flush = useCallback(async () => {
        ...
        const run = putDemoFiles(slug, batch).then(
            () => {
                setSavedFiles((current) => ({ ...current, ...batch }));
                setSaveStatus("saved");
            },
            () => {
                // Put the batch back so the next edit retries it.
    ...
    // Save whatever is pending when the tab is hidden or closed.
    useEffect(() => {
        const onHide = () => {
    ...
    return (
        <div className="flex h-screen w-screen flex-col overflow-hidden bg-background text-foreground">
            <header className="flex h-11 shrink-0 items-center gap-3 border-b px-3">
                <Button variant="ghost" size="icon" render={<a href="#/" aria-label="All demos" />}>
