 ...
 * package (`dist/schema/demo.config.json`, emitted by `scripts/emit-json-schema.mjs`
 * at build time), so the published schema can never drift from what the
 * player validates. This module deliberately holds only the URL string so it
 * stays a zero-dependency import for the tools that bake it into configs.
 */

/** Base URL for published schema documents. */
const SCHEMA_BASE =
  'https://cdn.jsdelivr.net/npm/@inkly-org/interactive-demo/dist/schema';
 ...
 * `https://cdn.jsdelivr.net/npm/@inkly-org/interactive-demo@<version>/dist/schema/demo.config.json`.
 */
